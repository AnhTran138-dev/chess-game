...
export function ChessPieceComponent({ piece, isSelected, canMove, isInCheck }: ChessPieceProps) {
  const pieceRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (pieceRef.current) {
      // Animation nhẹ nhàng từ scale nhỏ lên bình thường, không ảnh hưởng đến opacity
      gsap.fromTo(pieceRef.current, { scale: 0.8 }, { scale: 1, duration: 0.2, ease: "back.out(1.2)" })
    }
  }, [piece])
  ...
  return (
    <div
      ...
      style={{
        filter: isSelected ? "brightness(1.2)" : isInCheck && piece.type === "king" ? "brightness(1.3)" : "none",
        textShadow:
          isInCheck && piece.type === "king"
